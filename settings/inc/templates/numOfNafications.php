...
$stmt = $con->prepare("SELECT * FROM naficationsstatus WHERE friend_id IN (SELECT user_id from friends WHERE user_id = ? UNION SELECT friend_id from friends WHERE friend_id = ?) AND readed=0 ");
